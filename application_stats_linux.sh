...
if [ -s "$isStartEmpty" ]; then
        ...
        start_time_am=$(date -d  "${getStartAMTime}" "+%s") #compute start time in Linux format
        start_time_taskcontainer=$(date -d "${getStartTaskContainerTime}" "+%s") #compute end time in Linux format
        ...
        isKilled=0
          while read -r startcont
                do
                  ...
                  then
                   start_time=$(date -d "${starttime}" "+%s") #compute start time in Linux format
                   end_time=$(date -d "${endtime}" "+%s") #compute end time in Linux format
